...
function animate() {
  ...
  // console.log(sounds[1].analyser.getAverageFrequency())
  // time += 1
  for (let sound of sounds) {
    let data = sound.analyser.getAverageFrequency()
    let scale = data.map(0.0, 12.0, 0.0, 1.0)
    // let scaleVec = new THREE.Vector3(scale)
    sound.mesh.material.color= new THREE.Color("hsl(" + Math.floor(scale * 100) + "%, 1.0, 0.5)")
  }
  // time += 0.5
  // analysers[0].color = new THREE.Color("hsl(" + time + ", 20, 100)")
  // console.log(analysers[0].obj.material.color)
